...
const checks = [
  ...
  ['boss energy telegraph exists', /boss-telegraph/.test(files.battleService) && /bossEnergy\s*>=\s*75/.test(files.battleService)],
  ['boss telegraph releases on the following round', /bossTelegraphRound\s*>\s*0/.test(files.battleService) && /session\.round\)\s*>\s*bossTelegraphRound/.test(files.battleService)],
  ['boss mechanic energy is independent from ordinary active skills', /cooldowns\.bossEnergy/.test(files.battleService) && !/boss\.energy\s*=\s*Math\.min\(100/.test(files.battleService)],
