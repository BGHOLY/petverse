...
/**
 * One shared display-name rule for pet detail, formation, battle, friends,
 * marriage pickers and rankings. Rarity/species remain separate UI fields.
 */
export function cleanPetDisplayName(pet: any, fallback = '宝宝', maxLength = 10) {
    ...
    const value = cleaned && !/^\d+$/.test(cleaned) ? (legacy || cleaned) : (legacy || speciesFallback);
